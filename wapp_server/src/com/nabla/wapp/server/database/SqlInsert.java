...
public class SqlInsert<T> {
	...
	public SqlInsert(final Class<T> recordClass, int autoGeneratedKeys) {
		this(recordClass, "INSERT INTO " + getTable(recordClass).name() + " ({0}) VALUES({1});", autoGeneratedKeys);
	}

	public SqlInsert(final Class<T> recordClass) {
		this(recordClass, Statement.NO_GENERATED_KEYS);
	}
	...
	public SqlInsert(final Class<T> recordClass, final String sqlTemplate, int autoGeneratedKeys) {
		...
		sql = MessageFormat.format(sqlTemplate, names.toString(), values.toString());
		if (log.isDebugEnabled())
			log.debug("SQL=" + sql);
	}

	public PreparedStatement prepareStatement(final Connection conn, int autoGeneratedKeys) throws SQLException {
	...
	public PreparedStatement prepareStatement(final Connection conn) throws SQLException {
		return prepareStatement(conn, autoGeneratedKeys);
	}

	public BatchInsertStatement<T> prepareBatchStatement(final Connection conn, int autoGeneratedKeys) throws SQLException {
		return new BatchInsertStatement<T>(conn, this, autoGeneratedKeys);
	}
	...
	private static IRecordTable getTable(final Class clazz) {
	...
	@SuppressWarnings("unchecked")
	private void buildParameterList(final Class clazz) {
		if (clazz != null) {
