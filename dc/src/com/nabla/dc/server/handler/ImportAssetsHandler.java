...
import com.nabla.dc.server.xml.assets.XmlAssets;
...
public class ImportAssetsHandler extends AbstractHandler<ImportAssets, StringResult> {
	...
	@Inject
	public ImportAssetsHandler(@IReadWriteDatabase final IDatabase writeDb) {
		super(true);
		this.writeDb = writeDb;
	}

	@Override
	public StringResult execute(final ImportAssets cmd, final IUserSessionContext ctx) throws DispatchException, SQLException {
	...
	private boolean add(final ImportAssets cmd, final IErrorList<Integer> errors, final IUserSessionContext ctx) throws DispatchException, SQLException {
		...
		final XmlAssets assets = new Importer(ctx.getReadConnection(), errors, new ImportContext(ctx.getReadConnection())).read(XmlAssets.class, cmd.getFileId());
		...
		try {
			...
			return guard.setSuccess(assets.save(guard.getConnection(), new SaveContext(guard.getConnection(), option, errors)));
		} finally {
			guard.close();
		}
	}
