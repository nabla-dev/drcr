...
public class ImportCompanyAssetsHandler extends AbstractHandler<ImportCompanyAssets, StringResult> {
	...
	private boolean add(final ImportCompanyAssets cmd, final ICsvErrorList errors, final IUserSessionContext ctx) throws DispatchException, SQLException {
		UserPreference.save(ctx, null, IImportAssets.PREFERENCE_GROUP, IImportAssets.OVERWRITE, cmd.getOverwrite());
		final XmlCompanyAssets assets = new Importer(ctx.getReadConnection(), errors, new ImportContext(cmd.getCompanyId())).read(XmlCompanyAssets.class, cmd.getFileId());
