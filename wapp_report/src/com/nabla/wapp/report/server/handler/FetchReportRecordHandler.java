...
public class FetchReportRecordHandler extends AbstractFetchHandler<FetchReportRecord> {
...
" WHERE r.id IN (?)"
	...
	@Override
	public FetchResult execute(final FetchReportRecord cmd, final IUserSessionContext ctx) throws DispatchException, SQLException {
		return sql.fetch(cmd, ctx.getConnection(), ctx.getLocale().toString(), cmd.getIds());
	}
