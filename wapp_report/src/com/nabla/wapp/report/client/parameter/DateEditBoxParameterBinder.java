...
import com.nabla.wapp.report.shared.parameter.DateParameterValue;
 ...
 *
 */
public class DateEditBoxParameterBinder extends TSimpleParameterBinder<DateEditBoxParameter, Date> {

	private static final DateTimeFormat	dateFormatter = DateTimeFormat.getFormat("yyyy-MM-dd");

	public DateEditBoxParameterBinder(final DateEditBoxParameter parameter, final Date value) {
		super(parameter, value);
	}

	@Override
	public boolean createModelField(List<DataSourceField> fields) {
		fields.add(new DateField(parameter.getName(), isReadOnly() ? FieldAttributes.READ_ONLY : FieldAttributes.REQUIRED));
		return true;
	}
	...
	public boolean createFormItem(Form form, ParameterBinderList binders) {
		...
		if (isReadOnly())
			form.setValue(parameter.getName(), dateFormatter.format(value));
		return true;
	}

	@Override
	public Map<String, String> getValue(ValuesManager manager, List<IParameterValue> values) {
		final Date dt = (Date) manager.getValue(parameter.getName());
		values.add(new DateParameterValue(parameter.getName(), dt, dateFormatter.format(dt)));
		return null;
	}
